...
execute positioned ^ ^ ^0.1 unless block ~ ~ ~ #.mw:through run scoreboard players set set tmp 1
execute positioned ^ ^ ^0.1 if entity @e[tag=player,tag=!died,tag=enemy,distance=..1.8] run scoreboard players set set tmp 1
execute positioned ^ ^ ^0.1 if score set tmp matches 1.. run function char:2-steve/private/skill_active_a_do
