...
execute if score x tmp >= xMin params if score y tmp >= yMin params if score z tmp >= zMin params if score x tmp <= xMax params if score y tmp <= yMax params if score z tmp <= zMax params run tag @s add in_target_area
